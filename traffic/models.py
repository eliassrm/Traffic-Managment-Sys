...
class Traffic(models.Model):
	...
	class Meta:
		...
		indexes = [
			...
			models.Index(fields=['camera', 'congestion_level', '-measured_at']),
		]
...
class TrafficArchive(models.Model):
	source_record_id = models.BigIntegerField(unique=True, db_index=True)
	camera = models.ForeignKey('cameras.Camera', on_delete=models.SET_NULL, blank=True, null=True)
	measured_at = models.DateTimeField(db_index=True)
	vehicle_count = models.PositiveIntegerField(default=0)
	avg_speed_kph = models.DecimalField(max_digits=5, decimal_places=2)
	congestion_level = models.CharField(max_length=20, choices=Traffic.CongestionLevel.choices, db_index=True)
	occupancy_percent = models.DecimalField(max_digits=5, decimal_places=2)
	...
	def __str__(self):
		return f'Archived traffic {self.source_record_id} ({self.measured_at:%Y-%m-%d %H:%M:%S})'
